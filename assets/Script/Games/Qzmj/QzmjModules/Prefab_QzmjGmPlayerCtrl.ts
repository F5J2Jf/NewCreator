...
let ctrl : Prefab_QzmjGmPlayerCtrl;
//模型，数据处理
class Model extends BaseModel{
	...
	getMyCards(){
		...
		let players=QzmjLogic.getInstance().players;
...
class View extends BaseView{
	...
	createMahjong (id, parent) : void {
		...
		let sprite = new cc.SpriteFrame(QzmjResMgr.getInstance().getCardTextureByValue(id));
		node.getChildByName("MaJiang").getComponent(cc.Sprite).spriteFrame = sprite;
...
//c, 控制
@ccclass
export default class Prefab_QzmjGmPlayerCtrl extends BaseCtrl {
	//这边去声明ui组件
	@property({
		tooltip : "我方的麻将位置",
		type : cc.Node
	...
	start () {
		let msg = {
			reqtype : QzmjDef.gmreq_cards, 
			data : {
				target : BehaviorMgr.getInstance().changeSeatId
			}
		};
		QzmjLogic.getInstance().gmReq(msg);
	}
	//网络事件回调begin
	...
	emitChangeCard () : void {
		if (this.model.handCard != null && this.model.oppositeCard != null) {
			let msg={ 
				optype : QzmjDef.gmop_changecard, 
				data:{
					src : this.model.handCard,
					dest : this.model.oppositeCard,
					seatId : BehaviorMgr.getInstance().changeSeatId
				}
			}
			QzmjLogic.getInstance().gmOp(msg);
			
			// let node = new cc.Node();//测试用的
			// cc.find("Canvas").addChild(node);
			// let s = new cc.SpriteFrame(QzmjResMgr.getInstance().getCardTextureByValue(this.model.handCard));
			// node.addComponent(cc.Sprite).spriteFrame = s;
			this.model.handCard = null;
			this.model.oppositeCard = null;
			BehaviorMgr.getInstance().changeSeatId = 0;
		}
	}
