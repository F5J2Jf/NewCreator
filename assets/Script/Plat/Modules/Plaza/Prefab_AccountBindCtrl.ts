...
let ctrl : Prefab_AccountBindCtrl;
//模型，数据处理
...
//c, 控制
@ccclass
export default class Prefab_AccountBindCtrl extends BaseControl {
	//这边去声明ui组件
	@property({
		tooltip : "关闭按钮",
		type : cc.Node
	...
	WeChatBind : cc.Node = null;

	@property({
		tooltip : "绑定手机按钮",
		type : cc.Node
	})
	MobileBind : cc.Node = null;
	//声明ui组件end
	...
	//end
	//按钮或任何控件操作的回调begin
	private _onClick_Close () : void {
		this.finish();
	}

	private _onClick_WeChatBind () : void {
		
	}

	private _onClick_MobileBind () : void {
