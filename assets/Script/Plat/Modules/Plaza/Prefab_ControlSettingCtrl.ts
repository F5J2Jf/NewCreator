...
export default class Prefab_ControlSettingCtrl extends BaseControl {
	...
	connectUi()
	{
		this.connect(G_UiType.button, this.ui.button_resetBtn, this._onClick_resetBtn, '恢复默认设置')
		...
		this.connect(G_UiType.toggle, this.ui.button_sssAutoMate, this.sssAutoMate_cb, '智能带牌设置')
	}
    ...
    private _onClick_resetBtn (event) : void {
		...
		//SettingMgr.getInstance().reqReset()
    }

    private changeAccount_cb(event){
    	//切换到登录界面
    	console.log('切换账号')
    	this.start_module(G_MODULE.Login)
    }
